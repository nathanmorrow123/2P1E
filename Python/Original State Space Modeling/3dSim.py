...
capture_spheres_plot, = ax.plot([], [], [], 'bo', markersize=capture_radius*100, alpha=0.1)
...
def update_ships(frame):
    ...
    if all(captured_by_cutter):
        ...
        for i in range(num_cutters):
            ...
            evader_position += speed_ratio * evader_direction * dt + noise
        ...
        for i in range(num_cutters):
            ...
            noise = noise_level * np.random.randn(3)
            cutter_positions[i] += direction / np.linalg.norm(direction) * dt + noise
        ...
        # Update capture spheres positions
...
# Create animation
ani = animation.FuncAnimation(fig, update_ships, frames=480, interval=25, blit=False)
...
ani.save('cutters_and_fugitive_3d.gif', writer='pillow', fps=30, dpi = 300)
