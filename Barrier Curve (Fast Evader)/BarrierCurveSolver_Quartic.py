...
def boundryCheck(t,x_E,y_E):
    ...
    return np.isclose(0,res)

def solve_quartic(x_P, x_E, last_y_E=None):
    if last_y_E is not None:
        if x_E < (x_P - 1):
            y_values = np.linspace(last_y_E, 1, 10000)  # Scan y_E from last highest to 1.0 
        else:
            y_values = np.linspace(np.sqrt(1 - (x_P - x_E)**2), 1, 10000)  # Scan y_E from -1.0 to 1.0
    else:
        if x_E < (x_P - 1):
    ...
    for y_E in y_values:
        ...
        if len(real_roots) == 4:
            max_y_E = y_E
            print(boundryCheck(np.min(real_roots),x_E,y_E))
...
def create_animation(image_files):
    ...
    ani.save('barrier_curve_animation.gif', dpi = 300, fps=1)
